...
namespace Netick.Transport
{
    ...
    public sealed unsafe class ENetTransport : NetworkTransport
    {
        ...
        internal void Send(ENetPeer* peer, IntPtr ptr, int length, ENetPacketFlag flags) => _outgoings.Enqueue(NetworkOutgoing.Create(peer, (byte*)ptr, length, flags));
        ...
        public override void Connect(string address, int port, byte[] connectionData, int connectionDataLength)
        {
            ...
            enet_host_connect(_host, &enetAddress, 0, 0);
        }
        ...
        {
            while (_networkEvents.TryDequeue(out var netEvent))
            {
